...
app.post("/api/chat/message", webAuth, async (req, res) => {
    try {
        ...
        // virtual id for website user
        const chatId = "web_" + userId;

        // create session if missing
        if (!userSessions[chatId]) {
            const user = await usersCol.findOne({ _id: new ObjectId(userId) });
            const service = user?.currentService;
            ...
            if (service === "sender") {
                ...
                return res.json({ reply: "Service not selected." });
            }
        }

        // intercept bot message
        let botReply = "";
        ...
        } else if (userSessions[chatId].type === "traveler") {
            await handleTravelerTextStep(chatId, message);
        } else if (userSessions[chatId].type === "tracking") {
            if (!isValidPhone(message)) botReply = "❌ Invalid phone number.";
            else botReply = "Tracking coming soon.";
        }

        bot.sendMessage = originalSend;

        res.json({ reply: botReply || "OK" });
...
// ------------------- startup log -------------------
console.log('✅ AirDlivers bot (webhook + auto-recovery) is running...');// bot.js - AirDlivers production bot (webhook + auto-recovery)
// package.json must have: { "type": "module" }
