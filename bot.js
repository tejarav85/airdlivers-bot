...
async function isUserSuspended(userId) {
  const s = await sendersCol.findOne({ userId, suspended: true });
  if (s) return s;
  const t = await travelersCol.findOne({ userId, suspended: true });
  return t || null;
}
    ...
    if (otherDoc.pendingMatchWith === myReqId) {
      ...
      try {
        await bot.sendMessage(
          myDoc.userId,
          `🤝 <b>Match Confirmed!</b>\nYou are now matched with a ${otherLabel}.\n\nYou can now chat here. Any normal message (not starting with /) will be forwarded to your match and visible to admin.`,
          { parse_mode: 'HTML' }
        );
        await bot.sendMessage(
          otherDoc.userId,
          `🤝 <b>Match Confirmed!</b>\nYou are now matched with a ${myLabel}.\n\nYou can now chat here. Any normal message (not starting with /) will be forwarded to your match and visible to admin.`,
          { parse_mode: 'HTML' }
        );
      } catch (e) { }
...
async function tryForwardChatMessage(chatId, text) {
  try {
    ...
    const pairInfo = myDoc.role === 'sender'
      ? `Sender <code>${escapeHtml(myDoc.requestId)}</code> → Traveler <code>${escapeHtml(otherDoc.requestId)}</code>`
      : `Traveler <code>${escapeHtml(myDoc.requestId)}</code> → Sender <code>${escapeHtml(otherDoc.requestId)}</code>`;
...
bot.on('callback_query', async (query) => {
  try {
    ...
    const chatId = query.message.chat.id;
        // 🚫 Suspension guard (callbacks)
    const suspended = await isUserSuspended(chatId);
    if (suspended) {
  ...
  const reqId = data.replace('adm_suspend_', '');

  const doc =
    await sendersCol.findOne({ requestId: reqId }) ||
...
const col = isSender ? sendersCol : travelersCol;
  ...
  await bot.sendMessage(
    ADMIN_GROUP_ID,
    `🚫 User suspended\nRequest ID: <code>${escapeHtml(reqId)}</code>`,
    { parse_mode: 'HTML' }
  );
}
  ...
  const doc =
    await sendersCol.findOne({ requestId: reqId, matchLocked: true }) ||
    await travelersCol.findOne({ requestId: reqId, matchLocked: true });

  if (!doc || !doc.matchedWith) return;

  const otherCol = doc.role === 'sender' ? travelersCol : sendersCol;
  const otherDoc = await otherCol.findOne({ requestId: doc.matchedWith });

  for (const col of [sendersCol, travelersCol]) {
    await col.updateMany(
      { requestId: { $in: [reqId, doc.matchedWith] } },
      {
        $set: {
          matchLocked: false,
          chatTerminated: true,
          terminatedAt: new Date(),
          terminationReason: 'Admin terminated the chat'
        },
...
bot.on('message', async (msg) => {
  try {
    ...
    // 🚫 Suspension guard
    const suspended = await isUserSuspended(chatId);
    if (suspended && !text.startsWith('/help') && !text.startsWith('/privacy')) {
      await bot.sendMessage(
        chatId,
...
async function handleSenderTextStep(chatId, text) {
  ...
  switch (sess.step) {
    ...
    case 'sender_email':
      ...
      return bot.sendMessage(
        chatId,
        '🛫 Enter Pickup Airport (From):\n\n✈️ Please enter the airport clearly.\nExample: "Dubai International Airport" or "DXB".\nAvoid spelling mistakes or nicknames.',
        { parse_mode: 'HTML' }
      );
